...
使用插件内脚本 `"$CLAUDE_PLUGIN_ROOT/skills/lark-md-to-doc/scripts/render_lark_doc.py"` 读取 Markdown，按段调用 lark-cli：
...
3. 使用 `$CLAUDE_PLUGIN_ROOT`，避免命中本机插件缓存目录导致路径不一致。
4. 若未设置 `CLAUDE_PLUGIN_ROOT`，请手动设置为插件目录（如 `~/.claude/plugins/bytecoding`）。

# 创建新文档并渲染
python3 "$CLAUDE_PLUGIN_ROOT/skills/lark-md-to-doc/scripts/render_lark_doc.py" --md ./doc.md --title "文档标题" [--folder-token <token>]

# 渲染到已有文档
python3 "$CLAUDE_PLUGIN_ROOT/skills/lark-md-to-doc/scripts/render_lark_doc.py" --md ./doc.md --doc-id <DOC_ID>
```
