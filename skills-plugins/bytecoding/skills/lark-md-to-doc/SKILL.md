...
使用仓库内脚本 `plugin/skills/lark-md-to-doc/scripts/render_lark_doc.py` 读取 Markdown，按段调用 lark-cli：
...
3. 使用仓库内脚本路径，避免命中本机插件缓存目录导致路径不一致。

# 创建新文档并渲染
python3 plugin/skills/lark-md-to-doc/scripts/render_lark_doc.py --md ./doc.md --title "文档标题" [--folder-token <token>]

# 渲染到已有文档
python3 plugin/skills/lark-md-to-doc/scripts/render_lark_doc.py --md ./doc.md --doc-id <DOC_ID>
```
