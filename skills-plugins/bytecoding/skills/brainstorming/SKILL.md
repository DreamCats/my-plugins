...
**若已提问，在继续下一步之前等待用户回答并确认理解正确。** 若未提问，需给出简短的理解确认后再继续。
...
**顺序约束**：若执行 Repotalk，在完成前禁止任何本地 Search/Glob/Grep/Read。若已误用本地搜索，必须明确说明并从步骤 2 重新开始。
...
**硬性约束**：若选择执行步骤 2，未完成 Repotalk 搜索前，禁止使用本地搜索工具（Search/Glob/Grep/Read）。若跳过步骤 2，需说明原因后再执行本地搜索。
...
**优先使用 LSP 定位**（符号/引用/跳转）以提高搜索效率：

- 在候选范围内用 LSP 查找定义、引用、实现
- 若 LSP 无法覆盖，再使用 Grep/Glob 补充
...
6. **最终推荐**
   ...
   默认采用该方案继续生成文档；如需调整请说明。
```

**重要**：可以一次性输出所有分节内容，但必须保持分节清晰；不必等待逐节确认。
