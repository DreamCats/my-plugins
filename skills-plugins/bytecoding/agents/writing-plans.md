...
你需要遵循 **强制的 4 步工作流程**（定义在 `skills/writing-plans/SKILL.md`），将 design.md 转换为 tasks.md，任务粒度每个 2-5 分钟。
...
使用 **byte-lsp**（基于 gopls 的 LSP 服务）进行精确符号搜索：

```javascript
// 搜索符号定义
byte-lsp/search_symbols({
  query: "CreateUser"
});

// 跳转到定义（需要文件路径和位置）
byte-lsp/go_to_definition({
  file_path: "internal/handlers/user_handler.go",
  line: 120,
  column: 10
});

// 查找所有引用
byte-lsp/find_references({
  file_path: "internal/handlers/user_handler.go",
  line: 120,
  column: 10
});
...
**byte-lsp 优势**：
- ✅ 基于 gopls，提供准确的 Go 语义分析
- ✅ 自动容错：行列号落在空白处时自动定位最近标识符
- ✅ 支持符号搜索、定义跳转、引用查找、代码诊断
...
**输出**：完成 tasks.md 生成后，报告完成并提供文件位置和摘要（总任务数、预计时间、阶段）。
